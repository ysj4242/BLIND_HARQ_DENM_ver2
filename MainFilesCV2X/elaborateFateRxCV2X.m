function [resultingList,resultingList_DENM,resultingList_CAM,stationManagement,sinrManagement] = elaborateFateRxCV2X(timeManagement,IDvehicleTXLTE,indexVehicleTX,neighborsID,sinrManagement,stationManagement,positionManagement,appParams,phyParams)
...
resultingList = zeros(0,6);
resultingList_DENM = zeros(0,5);% Initialize error matrix
resultingList_CAM = zeros(0,5);% Initialize error matrix
...
%         end
        ...
        if sinrManagement.cumulativeSINR(IDvehicleRX,IDvehicleTXLTE(i)) >= sinrThreshold
            ...
            resultingList(indexRaw,6) = sinrManagement.cbrCV2X(IDvehicleTXLTE(i)); % COLUMN 5=1 IS "CORRECT"
            if stationManagement.DENM_pck(IDvehicleTXLTE(i)) == 1
                resultingList_DENM(indexRaw,1) = IDvehicleTXLTE(i);
                resultingList_DENM(indexRaw,2) = IDvehicleRX;
                resultingList_DENM(indexRaw,3) = stationManagement.BRid(IDvehicleTXLTE(i),stationManagement.pckTxOccurring(IDvehicleTXLTE(i)));
                resultingList_DENM(indexRaw,4) = distance(indexVehicleTX(i),stationManagement.activeIDsCV2X==IDvehicleRX);
                resultingList_DENM(indexRaw,5) = 1; % COLUMN 5=1 IS "CORRECT"
            elseif stationManagement.DENM_pck(IDvehicleTXLTE(i)) ~= 1
                resultingList_CAM(indexRaw,1) = IDvehicleTXLTE(i);
                resultingList_CAM(indexRaw,2) = IDvehicleRX;
                resultingList_CAM(indexRaw,3) = stationManagement.BRid(IDvehicleTXLTE(i),stationManagement.pckTxOccurring(IDvehicleTXLTE(i)));
                resultingList_CAM(indexRaw,4) = distance(indexVehicleTX(i),stationManagement.activeIDsCV2X==IDvehicleRX);
                resultingList_CAM(indexRaw,5) = 1; % COLUMN 5=1 IS "CORRECT"
            end
...
%                 fclose(fid);
        if find(stationManagement.pckTxOccurring ==4)
            disp(1)
        end
        elseif stationManagement.pckTxOccurring(IDvehicleTXLTE(i))>=stationManagement.cv2xNumberOfReplicas(IDvehicleTXLTE(i))
            ...
            resultingList(indexRaw,6) = sinrManagement.cbrCV2X(IDvehicleTXLTE(i)); % COLUMN 5=0 IS "ERROR"
            if stationManagement.DENM_pck(IDvehicleTXLTE(i)) == 1
                resultingList_DENM(indexRaw,1) = IDvehicleTXLTE(i);
                resultingList_DENM(indexRaw,2) = IDvehicleRX;
                resultingList_DENM(indexRaw,3) = stationManagement.BRid(IDvehicleTXLTE(i),stationManagement.pckTxOccurring(IDvehicleTXLTE(i)));
                resultingList_DENM(indexRaw,4) = distance(indexVehicleTX(i),stationManagement.activeIDsCV2X==IDvehicleRX);
                resultingList_DENM(indexRaw,5) = 0; % COLUMN 5=0 IS "ERROR"
            elseif stationManagement.DENM_pck(IDvehicleTXLTE(i)) ~= 1
                resultingList_CAM(indexRaw,1) = IDvehicleTXLTE(i);
                resultingList_CAM(indexRaw,2) = IDvehicleRX;
                resultingList_CAM(indexRaw,3) = stationManagement.BRid(IDvehicleTXLTE(i),stationManagement.pckTxOccurring(IDvehicleTXLTE(i)));
                resultingList_CAM(indexRaw,4) = distance(indexVehicleTX(i),stationManagement.activeIDsCV2X==IDvehicleRX);
                resultingList_CAM(indexRaw,5) = 0; % COLUMN 5=0 IS "ERROR"
            end
...
delIndex_DENM = resultingList_DENM(:,1)==0;
delIndex_CAM = resultingList_CAM(:,1)==0;
%if sum(delIndex)>0
%    error('Error strange here...');
%end
resultingList(delIndex,:) = [];
resultingList_DENM(delIndex_DENM,:) = [];
resultingList_CAM(delIndex_CAM,:) = [];
