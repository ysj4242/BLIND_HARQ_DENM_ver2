...
[simParams,appParams,phyParams,outParams] = initiateParameters({'default','Technology','NR-V2X','TypeOfScenario','PPP', ...
    ...
    'vMean',70,'vStDev',3,'roadWidth',3.5,'Raw',[50 75 100 125 150 200 300],'printUpdateDelay', true, 'seed', 2, 'dynamicScheduling', true, 'TsensingPeriod', 1, ...
    'cv2xNumberOfReplicasMax', 3, 'DENM_prob', 0.2, 'dcc_active', false, 'sizeSubchannel',25, 'BwMHz',10, 'SCS_NR',15, ...
    'Priority_SPS',false, 'T2autonomousMode_min',5,'resourceReEvaluation',false,'cv2xCbrFactor',1, 'Priority_DCC', 3, ...
    'generationIntervalAverageRandomPart', 0.01, 'ADT', false, 'ADT_thr', 1});
...
stationManagement.cv2xNumberOfReplicas(stationManagement.activeIDs(stationManagement.maxCAM+1:end,:)) = phyParams.cv2xNumberOfReplicasMax;
...
%             %If this is not the case, the pckNextAttempt must be reset
            if stationManagement.pckBuffer(idEvent)<=1
                ...
                if stationManagement.DENM_pck(idEvent) == 1
...
%                 stationManagement.DENM_pck(idEvent) = 0;
        ...
        generationInterval_DENM = timeManagement.generationIntervalDeterministicPart(idEvent);
        % generationInterval_DENM = timeManagement.generationIntervalDeterministicPart(idEvent) + exprnd(appParams.generationIntervalAverageRandomPart);
        if generationInterval >= timeManagement.dcc_minInterval(idEvent)
            if stationManagement.DENM_pck(idEvent) == 0
                timeManagement.timeNextPacket(idEvent) = round(timeManagement.timeNow + generationInterval, 10); % 선택된 timeEvent에 0.1 더함
            else
                timeManagement.timeNextPacket(idEvent) = round(timeManagement.timeNow + generationInterval_DENM, 10);
            end
        elseif generationInterval < timeManagement.dcc_minInterval(idEvent) && stationManagement.DENM_pck(idEvent) == 0
