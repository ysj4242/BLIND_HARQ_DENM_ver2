...
stationManagement.pckBuffer = zeros(simValues.maxID,2);

% Type of packets
% 1 = CAM
% 2 = DENM
stationManagement.pckType = constants.PACKET_TYPE_CAM * ones(simValues.maxID,1);
stationManagement.pckType_D = constants.PACKET_TYPE_DENM * ones(simValues.maxID,1);
...
sinrManagement.a = ones(simValues.maxID,1);
...
stationManagement.DENM_pck = zeros(simValues.maxID,1);

stationManagement.pckBuffer_prob = zeros(simValues.maxID,1);

stationManagement.prob = ones(simValues.maxID,1);

stationManagement.DENM_list = zeros(simValues.maxID,1);

stationManagement.a = 1;
stationManagement.b = 1;

%%%%
% 1열 : timeNextEvent 2열 : RC 3열 : BRid1 4열 : BRid2 5열 : timeNextPacket
stationManagement.saved = zeros(length(stationManagement.activeIDs),5);
...
% HARQ init
stationManagement.cv2xNumberOfReplicas = ones(simValues.maxID,1);
...
if appParams.variabilityGenerationInterval == constants.PACKET_GENERATION_ETSI_CAM
    ...
    timeManagement.generationIntervalDeterministicPart = appParams.generationInterval ...
                                                        - appParams.variabilityGenerationInterval/2 ...
                                                        + appParams.variabilityGenerationInterval*rand(simValues.maxID,1);
...
timeManagement.timeNextPacket(stationManagement.activeIDs) =...
    round(timeManagement.generationIntervalDeterministicPart(stationManagement.activeIDs)...
    .* rand(length(stationManagement.activeIDs),1), 10);
   ...
   if ismember(simParams.BRAlgorithm, [constants.REASSIGN_BR_REUSE_DIS_SCHEDULED_VEH,...
    ...
    if simParams.BRAlgorithm == constants.REASSIGN_BR_STD_MODE_4
        ...
        if simParams.dynamicScheduling == true
            maxDENM = simValues.maxID * 0.2;
            stationManagement.maxCAM = simValues.maxID-maxDENM;
            stationManagement.resReselectionCounterCV2X((simValues.maxID-maxDENM)+1:simValues.maxID) = ones;
            stationManagement.DENM_pck((simValues.maxID-maxDENM)+1:simValues.maxID) = ones;
        end
        ...
        for j=1:phyParams.cv2xNumberOfReplicasMax
            ...
            if j ==1 && any(stationManagement.DENM_pck(stationManagement.activeIDs) == 0)
                [stationManagement.BRid(stationManagement.activeIDs,j),~] = ...
                    BRreassignmentRandom(simParams.T1autonomousModeTTIs,simParams.T2autonomousModeTTIs,stationManagement.activeIDs,simParams,timeManagement,sinrManagement,stationManagement,phyParams,appParams);
            elseif j >1 && any(stationManagement.DENM_pck(stationManagement.activeIDs) == 1)
                [stationManagement.BRid(stationManagement.activeIDs(stationManagement.maxCAM+1:end),j),~] = ...
                    BRreassignmentRandom(simParams.T1autonomousModeTTIs,simParams.T2autonomousModeTTIs,stationManagement.activeIDs(stationManagement.maxCAM+1:end),simParams,timeManagement,sinrManagement,stationManagement,phyParams,appParams);
            end
        end      
        ...
        stationManagement.BRid(1:stationManagement.maxCAM,2:3) = -1;
        stationManagement.BRid = sort(stationManagement.BRid,2);
        stationManagement.BRid(1:stationManagement.maxCAM,1) = stationManagement.BRid(1:stationManagement.maxCAM,3);
        stationManagement.BRid(1:stationManagement.maxCAM,3) = -1;
